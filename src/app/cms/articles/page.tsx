...
export default function ArticleManagement() {
  ...
  const handleAddArticle = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!content || content === '<p><br></p>') {
      toast({ variant: "destructive", title: "Konten Kosong", description: "Silakan isi konten artikel." });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Artikel Berhasil Dibuat", description: "Wawasan baru telah ditambahkan." });
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Apakah Anda yakin ingin menghapus artikel ini?')) {
      ...
      toast({ title: "Artikel Dihapus", description: "Artikel telah dihapus dari sistem." });
      loadArticles();
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* CSS Fix untuk Numerik & List di Jodit */}
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-headline font-bold">Manajemen Artikel</h2>
          <p className="text-muted-foreground">Kelola wawasan teknis dan edukasi dengan fitur SEO lengkap.</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-5xl overflow-y-auto max-h-[95vh]">
            <DialogHeader>
              <DialogTitle>Buat Artikel Wawasan Baru</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleAddArticle} className="space-y-6 pt-4">
              ...
              <div className="space-y-2">
                ...
                <Input name="title" placeholder="Masukkan judul yang informatif..." required />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input name="tags" placeholder="contoh: iot, edukasi, transformasi digital" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold flex items-center gap-2">
                    <Key className="w-4 h-4 text-primary" /> Kata Kunci SEO
                  </label>
                  <Input name="keywords" placeholder="Keyword pencarian..." />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-semibold">Ringkasan (Excerpt)</label>
                <Input name="excerpt" placeholder="Ringkasan isi artikel..." required />
              </div>
      ...
      <Card className="shadow-md border-0">
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={4} className="text-center py-20 text-muted-foreground">Memuat daftar artikel...</TableCell></TableRow>
              ) : articles.length === 0 ? (
                <TableRow><TableCell colSpan={4} className="text-center py-20 text-muted-foreground font-medium">Belum ada artikel yang diterbitkan.</TableCell></TableRow>
              ) : (
                articles.map((item) => (
